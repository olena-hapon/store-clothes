...
const handlerAddToFavorites = (id: string) => {
...
useEffect(() => {
  ...
  //   let cardTransform = scroll * cardWidth + gap * scroll;
  //    setSliderWidth(cardTransform);

  //    return () => {window.onresize = false};

  //  }, [scroll, windowWidth])

  // let transform = `translateX(-${sliderWidth}px)`;
...
//  const clickNext = () => {
//   if (scroll === newProducts.length - 4) {
//     return;
//   }
//   setScroll(scroll + 1);
//  };
        ...
        >
          {!!newProducts && (
            ...
            <li className="carousel__item" ref={card} key={index}>
              ...
              <div
                  className="favorites"
                  onClick={(event) => handlerAddToFavorites(product.id)}
                >
                  {isInFavorite && id === product.id ? (
