...
import { setToogleNenuSearch } from '../../redux/slices/filter';
...
const Search = () => {
  ...
  return (
    <div className={!!clickInput ? 'search search__top' : 'search'}>
      {!clickInput ? (
        <input
          ...
          onClick={() => {setClickInput(!clickInput); dispatch(setToogleNenuSearch(true))}}
        />
        ) : (
          <div >
    ...
    {clickInput && (
      <SearchDropdown setClickInput={setClickInput} clickInput={clickInput}/>
    )}
    </div>
  )
}
